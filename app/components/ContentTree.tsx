...
const ContentTree = ({ dir, indentLevel = 0 }: { dir: Directory, indentLevel?: number, }) => {
    ...
    {indentLevel ? <li className="pt-1 capitalize text-(--hard-text-dark)">
            {dir.index ? <Link style={{paddingLeft: `${indentLevel}em`}} className={dir.index.route === location.pathname ? "font-semibold" : ""} to={dir.index.route}>{`${sectionNum} ${parsedName}`}</Link> : `${sectionNum} ${parsedName}`}
        </li> : null}
            ...
            switch (node.type){
                case "File":
                    ...
                    return <li key={node.fsPath}  className="pt-1 capitalize text-(--hard-text-dark)">
                        <Link style={{paddingLeft: `${indentLevel + 1}em`}} className={node.route === location.pathname ? "font-semibold" : ""} to={node.route}>{`${sectionNum} ${parsedName}`}</Link>
                    </li>
                case "Directory":
